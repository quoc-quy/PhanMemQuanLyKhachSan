...
CREATE TABLE LichSuChuyenPhong (
	MaChuyenPhong NVARCHAR(6) PRIMARY KEY,
	MaPhieuDatPhong NVARCHAR(20),
    PhongCu NVARCHAR(6),
    PhongMoi NVARCHAR(6),
    LyDo NVARCHAR(50),
	NgayChuyenPhong DATE,
	CONSTRAINT FK_LichSuChuyenPhong_PhieuDatPhong FOREIGN KEY (MaPhieuDatPhong) REFERENCES [dbo].[PhieuDatPhong](MaPhieuDatPhong)
);
UPDATE TAIKHOAN
SET MatKhau = LOWER(CONVERT(VARCHAR(40), HASHBYTES('SHA1', CAST(MatKhau AS VARCHAR(255))), 2))